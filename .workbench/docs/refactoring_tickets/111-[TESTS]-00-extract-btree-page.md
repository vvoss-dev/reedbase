# TESTS-111-00: Extract Tests from btree/page
...
**File**: btree/page (669 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `btree/page_test.rs`
...
rg "#\[cfg\(test\)\]" src/btree/page.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/btree/page.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/btree/page_test.rs

# OR after 002-[STRUCT]-00:
# cat src/store/btree/page.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/store/btree/page_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/btree/page.rs
...
- [ ] Test file created: `src/btree/page_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/btree/page.rs`
...
- `src/btree/page_test.rs`

**Modified** (current paths):
- `src/btree/page.rs` (remove inline tests)
- `src/btree/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/store/btree/page_test.rs`
- `src/store/btree/page.rs`
