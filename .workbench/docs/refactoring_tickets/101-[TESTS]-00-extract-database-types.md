# TESTS-101-00: Extract Tests from database/types
...
- **Current**: `src/database/types.rs` (before 002-[STRUCT]-00)
- **After**: `src/api/db/types.rs` (after 002-[STRUCT]-00)

Use current path if folder reorganisation not yet done.

## Context

**File**: database/types (570 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `database/types_test.rs`
...
rg "#\[cfg\(test\)\]" src/database/types.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/database/types.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/database/types_test.rs

# OR after 002-[STRUCT]-00:
# cat src/api/db/types.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/api/db/types_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/database/types.rs
...
- [ ] Test file created: `src/database/types_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/database/types.rs`
...
- `src/database/types_test.rs`

**Modified** (current paths):
- `src/database/types.rs` (remove inline tests)
- `src/database/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/api/db/types_test.rs`
- `src/api/db/types.rs`
- `src/api/db/mod.rs`
