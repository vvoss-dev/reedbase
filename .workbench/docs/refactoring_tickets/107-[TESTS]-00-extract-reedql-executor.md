# TESTS-107-00: Extract Tests from reedql/executor
...
**File**: reedql/executor (697 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `reedql/executor_test.rs`
...
rg "#\[cfg\(test\)\]" src/reedql/executor.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/reedql/executor.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/reedql/executor_test.rs

# OR after 002-[STRUCT]-00:
# cat src/api/reedql/executor.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/api/reedql/executor_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/reedql/executor.rs
...
- [ ] Test file created: `src/reedql/executor_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/reedql/executor.rs`
...
- `src/reedql/executor_test.rs`

**Modified** (current paths):
- `src/reedql/executor.rs` (remove inline tests)
- `src/reedql/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/api/reedql/executor_test.rs`
- `src/api/reedql/executor.rs`
