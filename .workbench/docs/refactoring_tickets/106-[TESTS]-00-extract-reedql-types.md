# TESTS-106-00: Extract Tests from reedql/types
...
- **Current**: `src/reedql/types.rs` (before 002-[STRUCT]-00)
- **After**: `src/api/reedql/types.rs` (after 002-[STRUCT]-00)

Use current path if folder reorganisation not yet done.

## Context

**File**: reedql/types (483 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `reedql/types_test.rs`
...
rg "#\[cfg\(test\)\]" src/reedql/types.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/reedql/types.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/reedql/types_test.rs

# OR after 002-[STRUCT]-00:
# cat src/api/reedql/types.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/api/reedql/types_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/reedql/types.rs
...
- [ ] Test file created: `src/reedql/types_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/reedql/types.rs`
...
- `src/reedql/types_test.rs`

**Modified** (current paths):
- `src/reedql/types.rs` (remove inline tests)
- `src/reedql/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/api/reedql/types_test.rs`
- `src/api/reedql/types.rs`
