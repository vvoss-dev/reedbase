# TESTS-113-00: Extract Tests from version/rebuild
...
- **Current**: `src/version/rebuild.rs` (before 002-[STRUCT]-00)
- **After**: `src/process/version/rebuild.rs` (after 002-[STRUCT]-00)

Use current path if folder reorganisation not yet done.

## Context

**File**: version/rebuild (200 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `version/rebuild_test.rs`
...
rg "#\[cfg\(test\)\]" src/version/rebuild.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/version/rebuild.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/version/rebuild_test.rs

# OR after 002-[STRUCT]-00:
# cat src/process/version/rebuild.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/process/version/rebuild_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/version/rebuild.rs
...
- [ ] Test file created: `src/version/rebuild_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/version/rebuild.rs`
...
- `src/version/rebuild_test.rs`

**Modified** (current paths):
- `src/version/rebuild.rs` (remove inline tests)
- `src/version/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/process/version/rebuild_test.rs`
- `src/process/version/rebuild.rs`
- `src/process/version/mod.rs`
