# TESTS-102-00: Extract Tests from database/execute
...
**File**: database/execute (661 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `database/execute_test.rs`
...
rg "#\[cfg\(test\)\]" src/database/execute.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/database/execute.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/database/execute_test.rs

# OR after 002-[STRUCT]-00:
# cat src/api/db/execute.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/api/db/execute_test.rs
```
...
rg "#\[cfg\(test\)\].*mod tests" src/database/execute.rs
...
- [ ] Test file created: `src/database/execute_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/database/execute.rs`
...
- `src/database/execute_test.rs`

**Modified** (current paths):
- `src/database/execute.rs` (remove inline tests)
- `src/database/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/api/db/execute_test.rs`
- `src/api/db/execute.rs`
- `src/api/db/mod.rs`
