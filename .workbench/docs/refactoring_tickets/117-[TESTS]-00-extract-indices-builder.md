# TESTS-117-00: Extract Tests from indices/builder
...
- **Current**: `src/indices/builder.rs` (before 002-[STRUCT]-00)
- **After**: `src/store/indices/builder.rs` (after 002-[STRUCT]-00)

Use current path if folder reorganisation not yet done.

## Context

**File**: indices/builder (400 lines)  
**Violation**: Has inline `#[cfg(test)] mod tests`  
**Required**: Extract to `indices/builder_test.rs`
...
rg "#\[cfg\(test\)\]" src/indices/builder.rs -A1 | grep "mod tests"
```
...
# Current path:
cat src/indices/builder.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/indices/builder_test.rs

# OR after 002-[STRUCT]-00:
# cat src/store/indices/builder.rs | awk '/#\[cfg\(test\)\]/,/^}$/ {print}' > src/store/indices/builder_test.rs
```
...
cargo test --lib indices::

# Verify no inline tests remain
rg "#\[cfg\(test\)\].*mod tests" src/indices/builder.rs
...
- [ ] Test file created: `src/indices/builder_test.rs`
- [ ] Copyright header added
- [ ] Inline tests removed from `src/indices/builder.rs`
...
- `src/indices/builder_test.rs`

**Modified** (current paths):
- `src/indices/builder.rs` (remove inline tests)
- `src/indices/mod.rs` (add test module reference)

**After 002-[STRUCT]-00**:
- `src/store/indices/builder_test.rs`
- `src/store/indices/builder.rs`
- `src/store/indices/mod.rs`
